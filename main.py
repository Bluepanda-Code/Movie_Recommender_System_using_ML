...
def load_data(movies_path='data/movies2.csv', ratings_path='data/ratings2.csv', links_path='data/links.csv', tags_path='data/tags.csv'):
    ...
    genre_counts.plot(kind='bar', color='#4C78A8', edgecolor='black')
...
def get_content_recommendations(movie_id, tags_grouped, cosine_sim, movie_indices, movies, links, ratings, n=10):
    if movie_id not in movie_indices:
        ...
        popular = popular[popular['userId'] > 20].sort_values('rating', ascending=False).sample(n, random_state=42)
...
def get_hybrid_recommendations(model, trainset, ratings, movies, links, tags_grouped, cosine_sim, movie_indices, user_id, n=10):
    try:
        ...
        # Normalize SVD scores to 0-5
        max_svd = top_svd['svd_score'].max()
        min_svd = top_svd['svd_score'].min()
        if max_svd != min_svd:
            top_svd['svd_score'] = (top_svd['svd_score'] - min_svd) / (max_svd - min_svd) * 5
        else:
...
def recommend():
    user_rating_plot = None
    recommended_genres_plot = None
    if request.method == 'POST':
        ...
        if user_id:
            try:
                ...
                if not user_ratings.empty:
                    plt.figure(figsize=(8, 6))
                    user_ratings.hist(bins=10, edgecolor='black', color='#4C78A8')
                    plt.title(f'Rating Distribution for User {user_id}')
                    plt.xlabel('Rating')
                    plt.ylabel('Count')
                    plt.savefig('static/user_rating_distribution.png')
                    plt.close()
                    user_rating_plot = 'user_rating_distribution.png'
            except Exception as e:
        ...
        genres = recommendations['genres'].str.split('|', expand=True).stack().reset_index(drop=True)
        ...
        plt.title('Genres in Recommended Movies')
        plt.savefig('static/recommended_genres.png')
        plt.close()
        recommended_genres_plot = 'recommended_genres.png'
        ...
        return render_template_string("""
            ...
            <head>
                ...
                <style>
                    ...
                    .plot-container img { max-width: 600px; }
                </style>
            </head>
            <body>
                ...
                <div class="plot-container">
                    <h2>Data Insights</h2>
                    {% if user_rating_plot %}
                    <h3>User {{ user_id }} Rating Distribution</h3>
                    <img src="/static/{{ user_rating_plot }}" alt="User Rating Distribution">
                    {% endif %}
                    {% if recommended_genres_plot %}
                    <h3>Genres in Recommended Movies</h3>
                    <img src="/static/{{ recommended_genres_plot }}" alt="Recommended Genres">
                    {% endif %}
                </div>
            </body>
            </html>
        ...
        user_rating_plot=user_rating_plot, recommended_genres_plot=recommended_genres_plot)
