...
def load_data(movies_path='data/movies2.csv', ratings_path='data/ratings2.csv', links_path='data/links.csv', tags_path='data/tags.csv'):
    ...
    # Print tag coverage
    print(f"Unique movies in tags.csv: {len(tags['movieId'].unique())}")
...
def get_content_recommendations(movie_id, tags_grouped, cosine_sim, movie_indices, movies, links, ratings, n=10):
    ...
    top_n = top_n.merge(links[['movieId', 'imdbId', 'tmdbId']], on='movieId', how='left')
    top_n['predicted_rating'] = top_n['movieId'].map(ratings.groupby('movieId')['rating'].mean()).fillna(3.5)
...
def get_hybrid_recommendations(model, trainset, ratings, movies, links, tags_grouped, cosine_sim, movie_indices, user_id, n=10):
    try:
        ...
        if max_svd != min_svd:
            top_svd['svd_score'] = (top_svd['svd_score'] - min_svd) / (max_svd - min_svd)
        else:
            top_svd['svd_score'] = 1.0
        ...
        if not user_ratings.empty:
            ...
            # Fallback to a random popular movie if user has no ratings
        ...
        if not content_recs.empty:
            ...
            if max_content != min_content:
                content_recs['content_score'] = (content_recs['content_score'] - min_content) / (max_content - min_content)
            else:
                content_recs['content_score'] = 1.0
        ...
        top_n['hybrid_score'] = 0.7 * top_n['svd_score'] + 0.3 * top_n['content_score']
    ...
    if request.method == 'POST':
        ...
        return render_template_string("""
            ...
            <head>
                ...
                <link rel="icon" type="image/x-icon" href="https://cdn.sanity.io/images/icgesbtv/production/732f53213b66ac2c9cf5818f38d7c848b3fe3a81-2048x1295.png?rect=0,71,2048,1152&w=1200&h=675&fit=crop&auto=format">
                <style>
            ...
            <body>
                ...
                <form method="post">
                    <label for="user_id">Enter User ID for Movie Recommendations:</label>
