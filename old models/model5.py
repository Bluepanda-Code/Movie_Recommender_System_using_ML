...
# Initialize TMDb API
tmdb = TMDb()
tmdb.api_key = os.getenv('TMDB_API_KEY', '64b1c927bbba4616649781093acc0ffd')  # Replace with your API key if not using env variable
tmdb.language = 'en'
movie_api = Movie()
...
def get_content_recommendations(movie_id, tags_grouped, cosine_sim, movie_indices, movies, links, n=10):
    ...
    top_n = tags_grouped.iloc[movie_indices_top][['movieId', 'title']]
    top_n = top_n.merge(movies[['movieId', 'genres']], on='movieId', how='left')
    top_n = top_n.merge(links[['movieId', 'imdbId', 'tmdbId']], on='movieId', how='left')
    top_n['predicted_rating'] = top_n['movieId'].map(ratings.groupby('movieId')['rating'].mean()).fillna(3.0)
    top_n['reason'] = f"Similar to movie ID {movie_id} based on tags"
    top_n['imdb_link'] = top_n['imdbId'].apply(lambda x: f"https://www.imdb.com/title/tt{x:07d}")
    top_n['details'] = top_n['tmdbId'].apply(get_movie_details)
    return top_n[['title', 'genres', 'predicted_rating', 'imdb_link', 'tmdbId', 'details', 'reason']]
...
def get_hybrid_recommendations(model, trainset, ratings, movies, links, tags_grouped, cosine_sim, movie_indices, user_id, n=10):
    try:
        ...
        top_svd = pd.DataFrame(svd_predictions[:n*2], columns=['movieId', 'svd_score'])
        ...
        if not user_ratings.empty:
            ...
            content_recs = get_content_recommendations(top_rated_movie, tags_grouped, cosine_sim, movie_indices, movies, links, n=n*2)
        ...
        top_n['content_score'] = top_n['content_score'].fillna(0)
        top_n['hybrid_score'] = 0.6 * top_n['svd_score'] + 0.4 * top_n['content_score']
        top_n = top_n.sort_values('hybrid_score', ascending=False).head(n)
        top_n = top_n.merge(movies[['movieId', 'title', 'genres']], on='movieId')
        top_n = top_n.merge(links[['movieId', 'imdbId', 'tmdbId']], on='movieId')
        top_n['reason'] = "Hybrid: SVD prediction + tag similarity"
        top_n['imdb_link'] = top_n['imdbId'].apply(lambda x: f"https://www.imdb.com/title/tt{x:07d}")
        top_n['details'] = top_n['tmdbId'].apply(get_movie_details)
        return top_n[['title', 'genres', 'hybrid_score', 'imdb_link', 'tmdbId', 'details', 'reason']]
    except ValueError:
        print(f"User {user_id} is new. Recommending popular movies.")
        return get_content_recommendations(1, tags_grouped, cosine_sim, movie_indices, movies, links, n=n)  # Fallback to content-based for movieId=1
    ...
    if request.method == 'POST':
        ...
        if user_id:
            ...
            try:
                ...
                recommendations = get_content_recommendations(movie_id, tags_grouped, cosine_sim, movie_indices, movies, links)
            except:
                movie_id = 1
                recommendations = get_content_recommendations(movie_id, tags_grouped, cosine_sim, movie_indices, movies, links)
        else:
